...
describe("Petstore Suite", () => {
  ...
  it("TC002 - Find pet by ID - Using valid ID", async () => {
    const response = await request("https://petstore.swagger.io").get(
      `/v2/pet/${petID}`
    );
    expect(response.statusCode).toBe(200);
    expect(response.body.name).toEqual("lion");
  ...
  it("TC005 - Update non-existing pet - Using invalid ID", async () => {
  ...
  it("TC006A - Delete existing pet - Using valid ID", async () => {
    const response = await request("https://petstore.swagger.io")
      .delete(`/v2/pet/${petID}`)
      .set("Content-Type", "application/json")
  ...
  it("TC006B - Delete a non-existing pet - Using invalid ID", async () => {
    const invalidID = "33333";
    const response = await request("https://petstore.swagger.io")
      .delete(`/v2/pet/${invalidID}`)
      .set("Content-Type", "application/json")
      .set("api_key", "special-key");
